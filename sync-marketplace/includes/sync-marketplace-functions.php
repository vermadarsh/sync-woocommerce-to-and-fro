 ...
 */
function smp_get_marketplace_woocommerce_client( $query_string_auth = false ) {
	...
	if ( empty( $consumer_key ) || empty( $consumer_secret ) || empty( $marketplace_url ) ) {
		return false;
	}
	...
	if ( true === $query_string_auth ) {
		$client_arr['query_string_auth'] = true;
	}

	return new Client( $marketplace_url, $consumer_key, $consumer_secret, $client_arr );
}

smp_get_marketplace_woocommerce_client();

/**
 * Prepare the customer data using customer ID. This data shall be exported to the marketplace.
 *
 * @param object $user Holds the user object.
 * @param int    $remote_customer_id Holds the remote customer ID.
 * @return array
 */
function smp_get_customer_data( $user, $remote_customer_id = '' ) {
	...
	$user_data = array(
		...
		'meta_data'  => array(
			...
			array(
				...
				'value' => smp_get_vendor_id_at_marketplace(),
			),
		),
	);
	...
	return apply_filters( 'smp_exported_customer_data', $user_data, $user );
}

/**
 * Get the term data that is posted to the remote WordPress URL.
 *
 * @param int $term_id Holds the term ID whose data is fetched.
 * @return boolean|array
 */
function smp_get_taxonomy_term_data( $term_id ) {
	...
	if ( 'product_cat' === $term->taxonomy ) {
		...
		if ( 0 !== $term->parent ) {
			$remote_parent_term = get_term_meta( $term->parent, 'synced_marketplace_with_id', true );
		}
	...
	return apply_filters( 'smp_exported_term_data', $term_data, $term );
}

/**
 * Prepare the attribute data which shall be posted to the remote marketplace.
 *
 * @param int   $attr_id Holds the attribute ID.
 * @param array $attr_data Holds the attribute data.
 * @return boolean|array
 */
function smp_prepare_product_attribute_data( $attr_id, $attr_data ) {
	...
	$attribute = array(
		...
		'synced_vendor_with_id'    => $attr_id,
		'smp_associated_vendor_id' => smp_get_vendor_id_at_marketplace(),
	);

	return apply_filters( 'smp_exported_product_attribute', $attribute, $attr_data );
 ...
 * @return void
 */
function smp_write_sync_log( $message = '' ) {
 ...
 */
function smp_get_image_src_by_id( $img_id ) {

	if ( empty( $img_id ) ) {
		return false;
	}

	return apply_filters( 'smp_image_src', wp_get_attachment_url( $img_id ), $img_id );

}

/**
 * Prepare the product data to be exported to marketplace.
 *
 * @param object $product Holds the woocommerce product object.
 * @return boolean|array
 */
function smp_get_product_data( $product ) {
	...
	if ( ! empty( $category_ids ) ) {
		foreach ( $category_ids as $category_id ) {
			$remote_category_id = get_term_meta( $category_id, 'synced_marketplace_with_id', true );
	...
	if ( ! empty( $tag_ids ) ) {
		foreach ( $tag_ids as $tag_id ) {
			$remote_tag_id = get_term_meta( $tag_id, 'synced_marketplace_with_id', true );
	...
	if ( ! empty( $upsell_product_ids ) && is_array( $upsell_product_ids ) ) {
		foreach ( $upsell_product_ids as $upsell_product_id ) {
			$remote_upsell_product_ids[] = get_post_meta( $upsell_product_id, 'synced_marketplace_with_id', true );
		}
	}
	...
	if ( ! empty( $cross_sell_product_ids ) && is_array( $cross_sell_product_ids ) ) {
		foreach ( $cross_sell_product_ids as $cross_sell_product_id ) {
			$remote_cross_sell_product_ids[] = get_post_meta( $cross_sell_product_id, 'synced_marketplace_with_id', true );
		}
	}
	...
	if ( 'simple' === $product_type || 'external' === $product_type || 'variable' === $product_type ) {
		foreach ( $attributes as $attribute ) {
			...
			$remote_attr_id = smp_get_remote_product_attribute_id( $attr_id );
	...
	if ( ! empty( $image_id ) ) {
		$product_image_src = smp_get_image_src_by_id( $image_id );
	}
	...
	if ( 'grouped' === $product_type ) {
		...
		if ( ! empty( $children_ids ) && is_array( $children_ids ) ) {
			foreach ( $children_ids as $children_id ) {
				$remote_product_id = get_post_meta( $children_id, 'synced_marketplace_with_id', true );
	...
	// Gather the marketplace product ID.
	$marketplace_product_id = get_post_meta( $product_id, 'synced_marketplace_with_id', true );

	if ( ! empty( $marketplace_product_id ) ) {
		$product_data['id'] = $marketplace_product_id;
	}

	return apply_filters( 'smp_exported_product_data', $product_data, $product );
}
 ...
 * @return boolean|string
 */
function smp_get_remote_product_attribute_id( $attr_id ) {
 ...
 * @return boolean|array
 */
function smp_get_remote_product_attribute_by_slug( $attr_slug ) {
...
/**
 * Prepare the coupon data to be exported to marketplace.
 *
 * @param object $coupon Holds the woocommerce coupon object.
 * @return boolean|array
 */
function smp_get_coupon_data( $coupon ) {
	...
	if ( ! empty( $product_ids ) && is_array( $product_ids ) ) {
		foreach ( $product_ids as $product_id ) {
			$remote_product_ids[] = get_post_meta( $product_id, 'synced_marketplace_with_id', true );
		}
	}
	...
	if ( ! empty( $excluded_product_ids ) && is_array( $excluded_product_ids ) ) {
		foreach ( $excluded_product_ids as $excluded_product_id ) {
			$remote_excluded_product_ids[] = get_post_meta( $excluded_product_id, 'synced_marketplace_with_id', true );
		}
	}
	...
	if ( ! empty( $product_categories ) && is_array( $product_categories ) ) {
		foreach ( $product_categories as $product_category ) {
			$remote_product_categories[] = get_term_meta( $product_category, 'synced_marketplace_with_id', true );
		}
	}
	...
	if ( ! empty( $excluded_product_categories ) && is_array( $excluded_product_categories ) ) {
		foreach ( $excluded_product_categories as $excluded_product_category ) {
			$remote_excluded_product_categories[] = get_term_meta( $excluded_product_category, 'synced_marketplace_with_id', true );
		}
	}
	...
	return apply_filters( 'smp_exported_coupon_data', $coupon_data, $coupon );
}

/**
 * Prepare the variation data to be exported to the marketplace.
 *
 ...
 * @return boolean|array
 */
function smp_get_variation_data( $variation, $remote_parent_id ) {
	...
	if ( ! empty( $attributes ) && is_array( $attributes ) ) {
		foreach ( $attributes as $taxonomy => $term_slug ) {
			...
			if ( ! empty( $term ) ) {
				$remote_attr = smp_get_remote_product_attribute_by_slug( $taxonomy );
	...
	if ( ! empty( $image_id ) ) {
		$variation_image_src = smp_get_image_src_by_id( $image_id );
	}
	...
	return apply_filters( 'smp_exported_variation_data', $variation_data, $variation );
}

/**
 * Prepare the order data to be exported to the marketplace.
 *
 * @param object $order Holds the woocommerce order object data.
 * @return boolean|array
 */
function smp_get_order_data( $order ) {
	...
	if ( ! empty( $shipping_methods ) && is_array( $shipping_methods ) ) {
		foreach ( $shipping_methods as $shipping_method ) {
			...
			if ( ! empty( wc_get_order_item_meta( $shipping_method->get_id(), 'synced_marketplace_with_id' ) ) ) {
				$temp['id'] = wc_get_order_item_meta( $shipping_method->get_id(), 'synced_marketplace_with_id' );
			}
	...
	$order_data = array(
		...
		'line_items'           => smp_get_order_line_items( $order->get_items() ),
	...
	return apply_filters( 'smp_exported_order_data', $order_data, $order );
}

/**
 * Prepare line items to be exported to the marketplace.
 *
 * @param object $line_items Holds the woocommerce line items object.
 * @return boolean|array
 */
function smp_get_order_line_items( $line_items ) {
	...
	foreach ( $line_items as $line_item_id => $line_item ) {
		...
		$remote_item_id = wc_get_order_item_meta( $line_item_id, 'synced_marketplace_with_id' );
		...
		if ( ! empty( $product_id ) ) {
			$temp['product_id'] = get_post_meta( $product_id, 'synced_marketplace_with_id', true );
		}
		...
		if ( ! empty( $variation_id ) ) {
			$temp['variation_id'] = get_post_meta( $variation_id, 'synced_marketplace_with_id', true );
		}
	...
	return apply_filters( 'smp_exported_order_line_items_data', $items, $line_items );
}
 ...
 */
function smp_get_product_review_data( $post_id, $comment ) {
 ...
 * @return boolean|array
 */
function smp_get_tax_rate_locations_by_id( $tax_rate_id ) {
	global $wpdb;
 ...
 * @return boolean|array
 */
function smp_get_tax_rate_data( $tax_rate_id, $tax_rate ) {
	...
	$tax_rate_locations = smp_get_tax_rate_locations_by_id( $tax_rate_id );
	$tax_rate_data      = array(
		...
		'synced_vendor_with_id'    => $tax_rate_id,
		'smp_associated_vendor_id' => smp_get_vendor_id_at_marketplace(),
	);
 ...
 * @return boolean|string
 */
function smp_get_remote_tax_rate_id( $tax_rate_id ) {
 ...
 * @return boolean|array
 */
function smp_get_webhook_data( $webhook_id, $webhook ) {
	...
	$webhook_data = array(
		...
		'synced_vendor_with_id'    => $webhook_id,
		'smp_associated_vendor_id' => smp_get_vendor_id_at_marketplace(),
	);
...
/**
 * Get remote webhook ID from source webhook ID.
 *
 * @param int $webhook_id Holds the webhook ID.
 * @return boolean|string
 */
function smp_get_remote_webhook_id( $webhook_id ) {
	...
	if ( ! empty( $saved_remote_webhooks ) && is_array( $saved_remote_webhooks ) ) {
		...
		if ( ! empty( $saved_remote_webhooks[ $webhook_id ]['remote_id'] ) ) {
			return $saved_remote_webhooks[ $webhook_id ]['remote_id'];
		}
	}
}

/**
 * Get remote shipping zone ID from source shipping zone ID.
 *
 * @param int $shipping_zone_id Holds the shipping zone ID.
 * @return boolean|string
 */
function smp_get_remote_shipping_zone_id( $shipping_zone_id ) {
	...
	if ( empty( $saved_remote_shipping_zones ) ) {
		return false;
	}

	if ( ! array_key_exists( $shipping_zone_id, $saved_remote_shipping_zones ) ) {
		return false;
	}

	if ( empty( $saved_remote_shipping_zones[ $shipping_zone_id ]['remote_id'] ) ) {
		return false;
	}

	return $saved_remote_shipping_zones[ $shipping_zone_id ]['remote_id'];
}
 ...
 * @return boolean
 */
function smp_is_user_customer( $user_id ) {
...
/**
 * Fetch the vendor ID at marketplace.
 *
 * @return string
 */
function smp_get_vendor_id_at_marketplace() {

	return get_option( 'smp_vendor_id_at_marketplace' );
}

if ( ! function_exists( 'smp_is_rest_api_request' ) ) {
	/**
	 * Function to check if the current request is a REST API request.
	 *
	 * @return boolean|int
	 */
	function smp_is_rest_api_request() {
 ...
 * @param int    $post_id Holds the post ID.
 */
function smp_set_featured_image_to_post( $image_url, $post_id ) {
 ...
 */
function smp_get_media_id_by_external_media_url( $image_url ) {
	$image_basename   = pathinfo( $image_url );
	$image_name       = $image_basename['basename'];
	$upload_dir       = wp_upload_dir();
	$image_data       = file_get_contents( $image_url );
 ...
 * @return boolean|array
 */
function smp_get_wc_product_attribute_taxonomies() {
 ...
 * @return array
 */
function smp_get_wp_wc_default_taxonomies() {
 ...
 * @return array
 */
function smp_get_wp_wc_taxonomies() {
	...
	$wc_attr_taxonomies = smp_get_wc_product_attribute_taxonomies();
 ...
 * @return array
 */
function smp_get_wp_wc_taxonomies_hooks() {
	$taxonomies     = smp_get_wp_wc_taxonomies();
	$taxonomy_hooks = array();

	if ( ! empty( $taxonomies ) && is_array( $taxonomies ) ) {
		foreach ( $taxonomies as $taxonomy ) {
			$taxonomy_hooks[ "{$taxonomy}_row_actions" ] = str_replace( '-', '_', "smp_{$taxonomy}_row_actions_callback" );
		}
	}
...
$taxonomy_hooks = smp_get_wp_wc_taxonomies_hooks();
	...
	foreach ( $taxonomy_hooks as $taxonomy_hook => $taxonomy_callback ) {
		add_filter(
			...
			function( $actions, $term ) {
				...
				$actions['smp-term-id'] = sprintf( __( 'ID: %1$s', 'sync-marketplace' ), $term->term_id );

				// Get the marketplace term ID.
				$marketplace_id = get_term_meta( $term->term_id, 'synced_marketplace_with_id', true );
				...
				$actions['smp-marketplace-term-id'] = sprintf( __( 'Marketplace ID: %1$s', 'sync-marketplace' ), $marketplace_id );
...
/**
 * Prepare the shipping method data to pe posted to marketplace.
 *
 * @param object $shipping_method Holds the shipping method object.
 */
function smp_get_shipping_zone_shipping_method_data( $shipping_method ) {
	...
	// Gather the shipping method settings data.
	foreach ( $shipping_method->get_instance_form_fields() as $id => $field ) {
		$data = array(
			'id'          => $id,
			'label'       => $field['title'],
			'description' => ( empty( $field['description'] ) ? '' : $field['description'] ),
			'type'        => $field['type'],
			'value'       => $shipping_method->instance_settings[ $id ],
	...
	// Gather the shipping method data.
 ...
 * @return boolean|array
 */
function smp_get_order_refund_data( $refund_id ) {
	...
	$refund_data = array(
		...
		'line_items'  => smp_get_refund_items( $refund->get_items() ),
 ...
 * @return boolean|array
 */
function smp_get_refund_items( $refund_items ) {
	...
	foreach ( $refund_items as $refund_item ) {
		...
		$remote_line_item_id = ( ! empty( $refund_line_item_id ) ) ? wc_get_order_item_meta( $refund_line_item_id, 'synced_marketplace_with_id' ) : '';

		// Get remote product ID.
		$product_id = $refund_item->get_product_id();
		$remote_product_id = ( ! empty( $product_id ) ) ? get_post_meta( $product_id, 'synced_marketplace_with_id', true ) : 0;

		// Get remote variation ID.
		$variation_id = $refund_item->get_variation_id();
		$remote_variation_id = ( ! empty( $variation_id ) ) ? get_post_meta( $variation_id, 'synced_marketplace_with_id', true ) : 0;
 ...
 * @return boolean|int
 */
function smp_get_order_id_by_refund_id( $refund_id ) {
 ...
 * @return string
 */
function smp_get_admin_error_message_html( $message ) {
	ob_start();
	?>
